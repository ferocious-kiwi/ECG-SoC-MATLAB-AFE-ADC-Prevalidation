...
function T = make_parameter_reference_table(p)
    ...
    add('Output stream','offset_binary_mem','%03X per line','hex','Physical ADC code reference; direct RTL use requires explicit convention matching or mid-code subtraction when downstream contract is signed 12-bit');
...
function T = make_input_dataset_manifest(classes, ref_dir, p)
    ...
    for i = 1:numel(classes)
        ...
        switch rec
            case 'NSR'
                source_database(i) = "MIT-BIH Normal Sinus Rhythm Database";
                source_record_id(i) = "16539";
                source_selection_note(i) = "Representative NSR input generated by upstream dataset flow; exact segment start is not tracked in this MATLAB repo.";
            case 'CHF'
                source_database(i) = "BIDMC Congestive Heart Failure Database";
                source_record_id(i) = "chf05";
                source_selection_note(i) = "Representative CHF input generated by upstream dataset flow; exact segment start is not tracked in this MATLAB repo.";
            case 'ARR'
                source_database(i) = "MIT-BIH Arrhythmia Database";
                source_record_id(i) = "105";
                source_selection_note(i) = "Representative ARR input generated by upstream dataset flow; exact segment start is not tracked in this MATLAB repo.";
            case 'AFF'
                source_database(i) = "MIT-BIH Atrial Fibrillation Database";
                source_record_id(i) = "04015";
                source_selection_note(i) = "Representative AFF input generated by upstream dataset flow; exact segment start is not tracked in this MATLAB repo.";
            otherwise
                source_database(i) = "not tracked in this MATLAB repo; reference input hash provided";
                source_record_id(i) = "not tracked in this MATLAB repo; reference input hash provided";
                source_selection_note(i) = "Source traceability is not tracked in this MATLAB repo; reference input hash provided.";
        end
        source_segment_start_s(i) = "not tracked in this MATLAB repo; reference input hash provided";
    ...
    T = table(input_class, source_database, source_record_id, source_segment_start_s, source_selection_note, ...
        expected_input_path, raw_source_file_in_repo, raw_source_input_sha256, ...
...
function s = compose_adc_mapping_doc(T)
    s = "# ADC Code Mapping Convention 정리\n\n" + ...
...
function s = compose_handoff_doc(p)
    s = sprintf(['# MATLAB-to-XMODEL 인계 문서\n\n' ...
